(function (vm, definition) {
  ...
  var SimpleSlider = function(containerElem, options) {
    ...
    var width = parseInt(vm.containerElem.style.width || vm.containerElem.offsetWidth, 10);

    // Get user defined options or its default values
    vm.trProp = getdef(options.transitionProperty, 'left');
    vm.trTime = getdef(options.transitionDuration, 0.5);
    vm.delay = getdef(options.transitionDelay, 3) * 1000;
    vm.unit = getUnit([options.startValue, options.visibleValue, options.endValue], vm.trProp);
    vm.startVal = parseInt(getdef(options.startValue, -width + vm.unit), 10);
    vm.visVal = parseInt(getdef(options.visibleValue, '0' + vm.unit), 10);
    vm.endVal = parseInt(getdef(options.endValue, width + vm.unit), 10);
    vm.autoPlay = getdef(parseStringToBoolean(options.autoPlay), true);
    vm.ease = getdef(options.ease, SimpleSlider.defaultEase);
    vm.onChange = getdef(options.onChange, null);
